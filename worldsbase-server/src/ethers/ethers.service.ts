...
export class EthersService {
  ...
  public async makeTransaction() {
    // abi I get from thirdweb dashboard
    const abi = [
      {
        type: 'event',
    ...
    const provider = new ethers.providers.JsonRpcProvider(
      'https://eth-sepolia.g.alchemy.com/v2/ixDCAKqEkG9eC4o5hWRkxNwln0mi-xMH',
    );

    // Our marketplace contract
    const contract = new ethers.Contract(
      '0xb6a470c31DC33E898B6fcadA53D97473fd852c35',
      abi,
      provider,
    );

    // contract of our direct listing item
    // this function works if you use contract.function.buyFromListing( 1,
    //   '0xE2dc27f386E713cd0F277151250811b401f30CB2',
    //   1,
    //   '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE',
    //   1,)

    const result = await contract.populateTransaction.buyFromListing(
      1,
      '0xE2dc27f386E713cd0F277151250811b401f30CB2',
      1,
      '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE',
    ...
    const messageHash = ethers.utils.arrayify(
      ethers.utils.keccak256(result.data as BytesLike),
    );

    const signature = await this.awsKmsService.sign(messageHash);
    // I don't check if signature is valid or not and couldn't manage to use it. Good luck!
