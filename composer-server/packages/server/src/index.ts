...
export class App {
    ...
    async config(io: Server) {
        ...
        this.app.get('/api/v1/workflows/:shortId', async (req: Request, res: Response) => {
            let skip = req.params?.skip ?? process.env.PAGINATION_DEFAULT_SKIP
            let limit = req.params?.limit ?? process.env.PAGINATION_DEFAULT_LIMIT
            const workflows: IWorkflowResponse[] = await this.AppDataSource.getMongoRepository(Workflow)
                .aggregate([
                    {
                        $match: {
...
export async function start(): Promise<void> {
    ...
    server.listen(port, () => {
        console.info(`⚡️[server]: Composer Server is listening at ${port}`)
    })
}
