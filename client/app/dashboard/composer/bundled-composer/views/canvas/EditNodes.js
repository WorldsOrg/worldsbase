...
import { Accordion, AccordionSummary, AccordionDetails, Box, ClickAwayListener, Divider, Paper, Stack, Popper, Typography, TextField, Avatar } from "@mui/material";
...
const EditNodes = ({ node, nodes, edges, workflow, onNodeLabelUpdate, onNodeValuesUpdate }) => {
  ...
  return (
    <>
      {/* Just using this div as an anchor */}
      <div ref={anchorRef} style={{ position: 'absolute', right: '40px', left: '40px', top: '70px', width: '1px', height: '1px', visibility: 'hidden' }}/>
      <Popper
              ...
              <ClickAwayListener onClickAway={handleClose}>
                <MainCard border={false} elevation={16} content={false} boxShadow shadow={theme.shadows[16]}>
                  ...
                  <PerfectScrollbar
                    ...
                    style={{ height: "100%", maxHeight: "calc(100vh - 400px)", overflowX: "hidden" }}
                  >
                    ...
                    {nodeFlowData && nodeFlowData.label && (
                      ...
                      >
                        <TextField id={nodeFlowData.name} label="Node Label" variant="outlined" value={nodeLabel} onChange={handleNodeLabelChange} fullWidth />
                        <StyledFab
                          sx={{
                            minHeight: 10,
                            height: 27,
                    ...
                    {nodeParamsType.includes("inputParameters") && (
                      <Box sx={{ p: 2 }}>
                        <Accordion expanded={expanded === "inputParameters"} onChange={handleAccordionChange("inputParameters")}>
                          <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="inputParameters-content" id="inputParameters-header">
                    ...
                    {nodeDetails && nodeFlowData && (
                      <Box sx={{ p: 2 }}>
                        <Accordion expanded={expanded === "outputResponses"} onChange={handleAccordionChange("outputResponses")}>
                          <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="outputResponses-content" id="outputResponses-header">
