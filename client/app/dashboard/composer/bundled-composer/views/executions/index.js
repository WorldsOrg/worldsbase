...
import { Box, List, Accordion, AccordionSummary, AccordionDetails, Paper, Popper, Chip, Stack, Typography, Button, IconButton } from "@mui/material";
...
const Executions = ({ workflowShortId, execution, executionCount, isExecutionOpen, anchorEl }) => {
  ...
  return (
    <>
                          ...
                          execution.map((exec, index) => (
                            <Box key={index}>
                              <Accordion expanded={expanded === exec.shortId} onChange={handleAccordionChange(exec.shortId)}>
                                <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls={`${exec.shortId}-content`} id={`${exec.shortId}-header`}>
                                      ...
                                      <div>
                                        {execData.data.map((execObj, execObjIndex) => (
                                          <div key={execObjIndex}>
                                            ...
                                            {execObj.html && (
                                              <StyledButton sx={{ mt: 1 }} size="small" variant="contained" onClick={() => openHTMLDialog(execData.data)}>
                                                View HTML
                                              </StyledButton>
                                            )}
                                            ...
                                            {execObj.attachments &&
                                              execObj.attachments.map((attachment, attchIndex) => (
                                                <div key={attchIndex}>
                                                  ...
                                                  <embed src={attachment.content} width="100%" height="100%" style={{ borderStyle: "solid" }} type={attachment.contentType} />
                                                  <StyledButton size="small" variant="contained" onClick={() => openAttachmentDialog(execData.data)}>
                                                    View Attachment
                                                  </StyledButton>
                                                </div>
                                              ))}
      ...
      <AttachmentDialog show={showAttachmentDialog} dialogProps={attachmentDialogProps} onCancel={() => setShowAttachmentDialog(false)}></AttachmentDialog>
      <HTMLDialog show={showHTMLDialog} dialogProps={HTMLDialogProps} onCancel={() => setShowHTMLDialog(false)}></HTMLDialog>
      <ExpandDataDialog show={showExpandDialog} dialogProps={expandDialogProps} onCancel={() => setShowExpandDialog(false)}></ExpandDataDialog>
    </>
  );
};
