...
import { Button, Dialog, DialogActions, DialogContent, IconButton, Box, List, Accordion, AccordionSummary, Typography, AccordionDetails } from "@mui/material";
...
const EditVariableDialog = ({ show, dialogProps, onCancel, onConfirm }) => {
  ...
  const onClipboardCopy = (e, node) => {
    ...
    if (copiedVariableBody) {
      ...
      else newInput = `${copiedVariableBody.textBeforeCursorPosition}{{${returnVariablePath}}}${copiedVariableBody.textAfterCursorPosition}`;
                      ...
                      {dialogProps.availableNodesForVariable.map((node, index) => (
                        <Box key={index}>
                          <Accordion expanded={expanded === node.data.label} onChange={handleAccordionChange(node.data.label)}>
                            <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls={`${node.data.label}-content`} id={`${node.data.label}-header`}>
                            ...
                            <AccordionDetails>
                              <div style={{ position: "relative" }}>
                                <ReactJson
                                  ...
                                  src={node.data.outputResponses && node.data.outputResponses.output ? node.data.outputResponses.output : {}}
